...
## Customer / Profile ✅ (10/05/2026)

- Path: `src/features/Profile/`
- Route: `/profile` (trong `ConsumerLayout`)

### Backend APIs (10/05/2026)
| Method | Endpoint | Mô tả |
|---|---|---|
| GET | `/v1/api/user/me` | Lấy profile hiện tại (map DB field → FE field) |
| PATCH | `/v1/api/user/me` | Cập nhật profile (map `user_gender`→`user_sex`, `user_birthday`→`user_date_of_birth`) |
| PATCH | `/v1/api/access/change-password` | Đổi mật khẩu (`currentPassword`, `newPassword`) |
| POST | `/v1/api/access/logout-all` | Đăng xuất tất cả thiết bị (xóa toàn bộ keyStore) |
| POST | `/v1/api/upload/user/avatar` | Upload avatar → Cloudinary + lưu URL vào `user_avatar` trong DB |
| POST | `/v1/api/shop/register` | Đăng ký cửa hàng (`name`, `address`, `contact`) |
| PATCH | `/v1/api/shop/:shopId` | Cập nhật thông tin shop (`shop_name`, `shop_contact`) |

### Frontend Services
- `userService.js` — `getMyProfileAPI`, `updateMyProfileAPI`, `changePasswordAPI`, `logoutAPI`, `uploadAvatarAPI`, `getMyAddressesAPI`, `getAddressDetailAPI`, `createAddressAPI`
- `shopService.js` — `getInfoShopAPI`, `getShopByUserAPI`, `registerShopAPI`, `updateShopAPI`

### Hooks
- `useProfile.js` — `useQuery(['my_profile', userId])` → `getMyProfileAPI`
- `useProfileMutation.js` — `updateProfile`, `changePassword`, `logoutAll` (→ `clearCurrentUser()` + `navigate('/login')`)
- `useOrderStats.js` — thống kê đơn hàng cho card Activity
- `useShop.js` — `useShop()` (query shop theo user), `useShopMutation()` (`registerShop`, `updateShop`, `isRegistering`, `isUpdatingShop`)

### Components
**`src/features/Profile/components/ProfilePage/`**
- `ProfileHeader.jsx` — banner gradient, avatar upload (`useUpload`), tên/email/chip shop, `ProfileActionButton`
- `PersonalInfoCard.jsx` — danh sách field (tên, email, phone, gender, birthday) + `FullEditProfileDialog`
- `FullEditProfileDialog.jsx` — RHF `useForm`; `Controller` cho MUI Select (giới tính); `reset()` khi dialog mở; validation required/pattern/max date
- `ChangePasswordDialog.jsx` — RHF `useForm`; `getValues` để validate confirmPassword; show/hide toggle cho 3 field; PASSWORD_RULE regex
- `SecurityCard.jsx` — list: đổi mật khẩu, logout-all, 2FA (disabled/coming soon), tài khoản liên kết (disabled)
- `ActivityStatsCard.jsx` — 3 StatBox (tổng đơn, đã chi, chờ xử lý), link đến orders, CTA shop
- `AddressCard.jsx` — danh sách địa chỉ user (từ `useAddress`), chip Mặc định, nút "Thêm địa chỉ" → mở `AddressModal` (tái dùng từ Order)
- `ShopInfoCard.jsx` — chỉ render khi `isShop`; avatar shop, tên, slug, chip trạng thái, 4 MetricBox (rating/sản phẩm/đã bán/followers), contact phone+email, nút Edit → `EditShopDialog` (RHF: tên shop, phone, email)

**`src/features/Profile/components/shared/`**
- `SectionTitle.jsx` — accent bar + icon + title
- `InfoRow.jsx` — label | value | verified badge
- `renderActionButton.jsx` (export `ProfileActionButton`) — React component có state; nếu chưa login → "Đăng nhập"; nếu isShop → "Quản lý Shop"; nếu user thường → "Mở cửa hàng" + mount `RegisterShopDialog`

**`src/features/Shop/components/`**
- `RegisterShopDialog.jsx` — RHF form: tên shop, SĐT, địa chỉ GHN cascading (Tỉnh→Huyện→Xã→số nhà); submit → `registerShop` mutation → cập nhật Redux `user_shop` ngay

### AppBar Profile Integration
- `AppBar.jsx` đọc `selectCurrentUser` từ Redux
- Có user: `Avatar` (ảnh hoặc chữ cái đầu) + click → `/profile`
- Không có user: Button "Đăng nhập" gradient → `/login`

### Field mapping BE↔FE
| Frontend field | DB field (user.model) |
|---|---|
| `user_gender` | `user_sex` |
| `user_birthday` | `user_date_of_birth` |
| `user_avatar` | `user_avatar` (same) |
| `user_name` | `user_name` (same) |
| `user_phone` | `user_phone` (same) |
