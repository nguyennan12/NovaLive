...
### Order Page (Consumer) ✅ (01/05/2026)
...
- `OrderAddressSection.jsx` — hiển thị địa chỉ mặc định (từ Redux user), nút "Chọn địa chỉ khác" → mở `AddressModal`
- `AddressModal.jsx` — Dialog nhập địa chỉ mới với React Hook Form (tên người nhận, SĐT, tỉnh/huyện/xã, địa chỉ chi tiết)
- `OrderShippingInfo.jsx` — logo GHN, phí ship (từ `useCheckout`), freeship chip, bảng chính sách vận chuyển
...
- `useCheckout(payload)` → tính tổng realtime (gọi `POST /order/checkout`)
- `useMutation(OrderByUserAPI)` → submit đặt hàng
- Mobile sticky bar dùng `createPortal(…, document.body)` — tránh bị Footer đè (WaterDropBackground tạo stacking context z-index:1)

**TODO còn lại:**
- `getUserAddressAPI` chưa có → địa chỉ mặc định lấy từ `user.default_address` (có thể null)
- Tỉnh/Huyện/Xã trong AddressModal là TextField tự nhập — cần thay bằng Select + API địa chỉ (GHN/VietnamProvinces)
- VNPay/MoMo sau submit cần redirect tới `response.paymentUrl`
- Sau đặt hàng thành công navigate về `/` tạm thời — cần trang `/orders/:orderId`
