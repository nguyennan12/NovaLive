...
import { uploadAvatarAPI, uploadShopLogoAPI } from '~/common/apis/services/uploadService'
...
const ProfileHeader = ({ profile, isLoading, isShop }) => {
  ...
  if (isLoading || (isShop && shopLoading)) return <ProfileHeaderSkeleton />

  const avatarSrc = isShop ? (shop?.shop_logo || '') : (profile?.user_avatar || '')
  const avatarLetter = isShop ? shop?.shop_name?.[0]?.toUpperCase() : profile?.user_name?.[0]?.toUpperCase()
  const displayName = isShop ? (shop?.shop_name || 'Cửa hàng') : (profile?.user_name || 'Người dùng')
          ...
          <Box sx={{ position: 'relative', display: 'inline-block', mt: '-48px', mb: 1 }}>
            <Avatar
              ...
              sx={{
                ...
                fontSize: '2.2rem',
                background: isShop
                  ? 'linear-gradient(135deg,#8b5cf6,#0095ff)'
                  : 'linear-gradient(135deg,#0095ff,#8b5cf6)'
              }}
            >
              {avatarLetter}
            </Avatar>
            <Tooltip title={isShop ? 'Đổi logo cửa hàng' : 'Đổi ảnh đại diện'}>
              <IconButton
                size="small"
                sx={{
                ...
                onClick={isShop ? uploadShopLogo.openFileDialog : uploadAvatar.openFileDialog}
              >
                <input {...(isShop ? uploadShopLogo.fileInputProps : uploadAvatar.fileInputProps)} />
                <CameraAltIcon sx={{ fontSize: 13 }} />
              </IconButton>
            </Tooltip>
          </Box>

          <Stack sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', gap: 1 }}>
            <Box>
              <Stack direction="row" alignItems="center" gap={0.75} sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            ...
            <Stack sx={{ display: 'flex', gap: 1, flexDirection: 'row', alignItems: 'flex-start', pt: { xs: 0, sm: 0.5 } }}>
              {isShop && (
                <Button
                  size="small"
                  variant="outlined"
              ...
              <ProfileActionButton currentUser={currentUser} isShop={isShop} navigate={navigate} />
            </Stack>
          </Stack>
        </Box>
      </Box>
