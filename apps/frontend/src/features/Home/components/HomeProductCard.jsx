  ...
  <Box sx={{
    ...
    <Box sx={{ p: { xs: 1.25, sm: 1.5 } }}>
      <Typography noWrap sx={{
        ...
        fontWeight: 700, color: 'primary.contrastText', mb: 0.85
      }}>
        {product.spu_name}
      </Typography>
...
const LandscapeCard = ({ product }) => (
  <Box sx={{
    ...
    <Box sx={{
      flex: 1, p: { xs: 1.25, sm: 1.75 },
      display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
      overflow: 'hidden'
    }}>
      <Typography sx={{
        fontSize: { xs: '0.85rem', sm: '0.92rem' }, fontWeight: 700,
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 0.75 }}>
        <Box>
          <Typography sx={{
            fontSize: { xs: '1rem', sm: '1.1rem' }, fontWeight: 800,
            color: 'secondary.main', letterSpacing: '-0.02em', lineHeight: 1
          }}>
            {formatVND(product.spu_price)}
          </Typography>
...
const HomeProductCard = ({ product, variant = 'portrait' }) => {
